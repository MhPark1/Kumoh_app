...
const GpsScreen = () => {
  ...
  const generateTripId = () => {
    ...
    const date = new Date();
    return crypto.SHA256(rand + ',' + date.toString()).toString(crypto.enc.Hex);
  };
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            // 센서 데이터 필터링 (시작~종료 시간)
            const filteredAccel = filterSensorData(
              accelData,
              startTimestamp,
              stopTimestamp,
            );
            const filteredGyro = filterSensorData(
              gyroData,
              startTimestamp,
              stopTimestamp,
            );
            const filteredMotionData = filterSensorData(
              motionData,
              startTimestamp,
              stopTimestamp,
            );
            const filteredCompasData = filterSensorData(
              headingData,
              startTimestamp,
              stopTimestamp,
            );
            ...
            // Columnar 변환
            const accelFormatted = arrayToColumnar(filteredAccel, [
              'x',
              'y',
              'z',
            ]);
            const gyroFormatted = arrayToColumnar(filteredGyro, [
              'x',
              'y',
              'z',
            ]);
            const motionFormatted = arrayToColumnar(filteredMotionData, [
              'pitch',
              'roll',
              'yaw',
            ]);
            const azimuthFormatted = arrayToColumnar(filteredCompasData, [
              'heading',
            ]);
            ...
            try {
              ...
              const send = {
                trip_id:
                  'e0ed07302d826b9f1774d78089f8d3deb7ebeee45bddda41e5aa4f',
  ...
  const handleHistory = () => {
    if (resultData) {
      Alert.alert('분석결과', '분석결과 화면으로 이동합니다.', [
        {
          text: 'OK',
      ...
      Alert.alert('분석결과 없음', '운행 분석 결과가 없습니다.', [
        {text: 'OK'},
      ]);
    }
  };
      ...
      <ScrollView>
        ...
        <View style={styles.warningContainer}>
          <Text style={styles.warningText}>
            • <Text style={styles.boldText}>급감속</Text>에 유의하세요
          </Text>
          <Text style={styles.warningText}>
            • <Text style={styles.boldText}>교통법규</Text>를 준수하세요
          </Text>
          <Text style={styles.warningText}>
            • <Text style={styles.boldText}>자전거 도로</Text>로 통행하세요
          </Text>
        </View>
